...
> **repo-specific (local environment):** the Wazuh dashboard repos sit side by side under the workspace root (the parent directory that contains the sibling dashboard checkouts). This repo is wazuh-dashboard-alerting. Write scratch files and issue drafts to a git-ignored dir such as `tmp/vuln-issues/`.

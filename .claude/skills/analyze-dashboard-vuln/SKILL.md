...
> **repo-specific (local environment):** the Wazuh dashboard repos sit side by side under the workspace root (on this machine `/home/asus/wazuh/`). This repo is wazuh-dashboard-alerting. Write scratch files and issue drafts to a git-ignored dir such as `tmp/vuln-issues/`.
